        ...
        <div class="scroll-container">
            ...
            >
                ...
                <div class="card-content">
                    <p class="api-note">POS~/api/graph/ontology/generate</p>
                    <p class="description">
            ...
            >
                ...
                <div class="card-content">
                    <p class="api-note">POS~/api/graph/build</p>
                    <p class="description">
                        基于生成的本体，将文档自动分块后调用 Zep
                        构建知识图谱，提取实体和关系，并形成时序记忆与社区摘要
                    </p>
...
const { currentPhase, projectData, ontologyProgress, buildProgress, graphData, systemLogs } =
    toRefs(props);
...
const canEnterEnvSetup = computed(() => {
    const hasGraph = !!(
        ...
        props.graphData?.node_count
    );
    return props.currentPhase >= 2 || hasGraph;
});

// 进入环境搭建 - 创建 simulation 并跳转
const handleEnterEnvSetup = async () => {
    if (!props.projectData?.project_id) {
        console.error('缺少项目信息');
        return;
    }
    ...
    try {
        let graphId = props.projectData?.graph_id;

        if (!graphId) {
            const projectRes = await getProject(props.projectData.project_id);
            if (projectRes.success && projectRes.data?.graph_id) {
                graphId = projectRes.data.graph_id;
            }
        }

        if (!graphId) {
            console.error('缺少图谱信息');
            return;
        }
