    ...
    <div class="main-view">
        ...
        <header class="app-header">
            <div class="header-left">
                <div class="brand" @click="router.push('/')">GarphRAG-Agent</div>
            </div>
...
const handleGoBack = async () => {
    ...
    // 返回到 Step 2 (环境搭建)
    router.push({ path: `/simulation/${currentSimulationId.value}` });
};
...
const startGraphRefresh = () => {
    addLog('开启图谱实时刷新 (30s)');
    // 立即刷新一次，然后每30秒刷新
    startPolling('graphRefresh', refreshGraph, 30000);
};

const stopGraphRefresh = () => {
    stopPolling('graphRefresh');
    addLog('停止图谱实时刷新');
};
