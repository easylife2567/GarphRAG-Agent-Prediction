...
PLAN_SYSTEM_PROMPT = """\
你是一个「传播机制复现与比较分析报告」的撰写专家，拥有对模拟世界的「上帝视角」——你可以洞察模拟中每一位Agent的行为、言论和互动。

【核心理念】
我们构建了一个可控模拟世界，并注入特定「模拟需求」作为条件变量。你观察的是传播过程复现数据，而不是现实实时数据。重点是解释传播机制，而不是预测未来。

【你的任务】
撰写一份「传播机制复现与比较分析报告」，回答：
1. 事件在各阶段如何演进与转折？
2. 各类Agent（人群）如何解释、放大、转向或固化信息？
3. 哪些机制最关键：框架迁移、中介放大、路径分化、议程变化？

【报告定位】
- ✅ 基于模拟复现数据进行机制解释
- ✅ 聚焦过程：阶段、转折、主体互动、跨平台差异
- ✅ 输出可用于跨案例比较的中层特征模式
- ❌ 不是未来预测报告
- ❌ 不是泛泛而谈的舆情综述
...
请以「上帝视角」审视这个传播过程复现：
1. 在设定条件下，传播过程呈现出什么阶段结构？
2. 各类人群（Agent）是如何反应和行动的？
3. 哪些机制模式最值得关注（转折、框架迁移、中介放大、路径分化）？

根据复现结果，设计最合适的报告章节结构。

【再次提醒】报告章节数量：最少2个，最多5个，内容要精炼聚焦于核心机制发现。"""

# ── 章节生成 prompt ──

SECTION_SYSTEM_PROMPT_TEMPLATE = """\
你是一个「传播机制复现与比较分析报告」的撰写专家，正在撰写报告的一个章节。

报告标题: {report_title}
报告摘要: {report_summary}
复现场景（模拟需求）: {simulation_requirement}
...
模拟世界是传播过程的受控复现。我们向模拟世界注入了特定条件（模拟需求），
模拟中Agent的行为和互动，是机制解释的证据。

你的任务是：
- 揭示在设定条件下，传播过程如何演进与转折
- 分析各类人群（Agent）如何重构信息与互动放大
- 发现值得关注的机制模式、边界条件和差异路径

❌ 不要写成对现实世界现状的分析
✅ 要聚焦于"传播机制如何运作"——模拟结果就是机制证据
    ...
    def plan_outline(
        ...
        try:
            ...
            return ReportOutline(
                title="传播机制复现与比较分析报告",
                summary="基于模拟复现的传播阶段、机制与路径分析",
                sections=[
                    ReportSection(title="复现场景与核心发现"),
                    ReportSection(title="关键主体行为与机制解释"),
                    ReportSection(title="路径分化与比较结论")
                ]
            )
